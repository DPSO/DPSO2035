...
private _presetName = format ["DPSO_preset%1", _network];
...

{
    if (player canAdd _radio) then {
        ...
        systemChat format ["[DPSO] Added radio: %1", _radio];
    } else {
        if (getContainerMaxLoad uniform player > 0) then {
            (uniformContainer player) addItemCargoGlobal [_radio, 1];
            systemChat format ["[DPSO] Added radio (exceeds inventory capacity): %1", _radio];
        } else {
            systemChat format ["[DPSO] Couldn't add radio: %1", _radio];
        };
