...
if (_type == 28) then {
    if (GVAR(respawnMousePos) isEqualTo -1) then {
        ...
        if (GVAR(respawnMousePos) isEqualType []) then {
            ...
            private _var = missionNamespace getVariable[format["dpso_respawnPoint%1",GVAR(respawnMousePos)],objNull];
            if (!isNull _var) then {
        ...
        GVAR(respawnGuiParameters) params ["_faction", "_groupName", "_markerType", "_markerColor", "_markerName"];
 {
        ...
        // Close the Map
        ((findDisplay IDC_DPSO_ADMINMENU_RESP_MAP_DISPLAY) displayCtrl IDC_DPSO_ADMINMENU_RESP_MAP_CONTROL) ctrlShow false;
        ((findDisplay IDC_DPSO_ADMINMENU_RESP_MAP_DISPLAY) displayCtrl IDC_DPSO_ADMINMENU_RESP_MAP_CONTROL) mapCenterOnCamera false;
        closeDialog IDC_DPSO_ADMINMENU_RESP_MAP_DISPLAY;
    };
};
