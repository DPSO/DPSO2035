 ...
 *	\z\dpso\addons\medical\functions\ace_medical\fn_actionRemoveTourniquet.sqf
 *	by Ojemineh
 *
...
if (!(missionNamespace getVariable ["DPSO_medical_consume_tourniquet", false])) then {
	...
	if (missionNamespace getVariable ["DPSO_medical_return_items", true]) then {

		private _holding = (missionNamespace getVariable ["DPSO_medical_return_tourniquets", 1]);

		if ( ({_x == "ACE_tourniquet"} count (items _patient)) >= _holding ) then {
			[_medic, "ACE_tourniquet", 1, [3,1,2], true] call DPSO_fnc_addItem;
		} else {
			if (_patient canAdd "ACE_tourniquet") then {
				[_patient, "ACE_tourniquet", 1, [1,3,2], true] call DPSO_fnc_addItem;
			} else {
				[_medic, "ACE_tourniquet", 1, [3,1,2], true] call DPSO_fnc_addItem;
			};
		};
...
[_patient, "activity", localize "STR_DPSO_Medical_Activity_Tourniquet_Removed", [[_medic, false, true] call ACE_common_fnc_getName]] call ACE_medical_fnc_addToLog;
[_patient, "activity_view", localize "STR_DPSO_Medical_Activity_Tourniquet_Removed", [[_medic, false, true] call ACE_common_fnc_getName]] call ACE_medical_fnc_addToLog;
