 ...
 *	\z\dpso\addons\medical\functions\ace_medical\fn_actionCheckPulseLocal.sqf
 *	by Ojemineh
 *
...
private _output = localize "STR_DPSO_Medical_Check_Pulse_Output_3";
private _outputEx = "";
private _logOutPut = localize "STR_DPSO_Medical_Check_Pulse_None";

if (_heartRate > 1) then {
    if ([_medic] call ace_medical_fnc_isMedic) then {
        _output = localize "STR_DPSO_Medical_Check_Pulse_Output_1";
        _logOutPut = format ["%1", round(_heartRate)];
    } else {
        _output = localize "STR_DPSO_Medical_Check_Pulse_Output_2";
...
if (_bodyPart in ["hand_l","hand_r"] && {[_patient, _bodyPart] call ACE_medical_fnc_hasTourniquetAppliedTo}) then {
    _output = localize "STR_DPSO_Medical_Check_Pulse_Output_3";
    _logOutPut = localize "STR_DPSO_Medical_Check_Pulse_None";
};

private _height = 3.0;

if (_inRevive) then {
	private _outputAdd = format [".<br/>%1", localize "STR_DPSO_Medical_Need_CPR_AED"];
...
if (_logOutPut != "") then {
    [_patient, "activity", localize "STR_ACE_Medical_Check_Pulse_Log", [[_medic] call ACE_common_fnc_getName, _logOutPut]] call ACE_medical_fnc_addToLog;
