 ...
 *	\z\dpso\addons\medical\functions\ace_medical\fn_treatment.sqf
 *	by Ojemineh
 *
...
if (missionNamespace getVariable ["DPSO_medical_stethoscope_enabled", false]) then {

	private _modifier = 0;

	if (_className in ["CheckPulse", "CheckBloodPressure", "CheckResponse", "CheckLimbs", "Diagnose"]) then {
		if ("DPSO_Stethoscope" in items _medic) then {
			_modifier = -2;
		} else {
			_modifier = +2;
		};
