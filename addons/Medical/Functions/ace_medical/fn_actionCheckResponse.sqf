 ...
 *	\z\dpso\addons\medical\functions\ace_medical\fn_actionCheckResponse.sqf
 *	by Ojemineh
 *
...
private _output = localize "STR_DPSO_Medical_Check_Response_Output";
private _outputEx = "";

if ([_patient] call ACE_common_fnc_isAwake) then {
	_outputEx = format ["<t color='#00ff00'>%1</t>", localize "STR_DPSO_Medical_Check_Response_Responsive"];
} else {
	_outputEx = format ["<t color='#ff0000'>%1</t>", localize "STR_DPSO_Medical_Check_Response_Unresponsive"];
};
...
private _log1 = [
	localize "STR_DPSO_Medical_Check_Response_Unresponsive",
	localize "STR_DPSO_Medical_Check_Response_Responsive"
] select ([_patient] call ACE_common_fnc_isAwake);
private _logOutput = format [localize "STR_DPSO_Medical_Check_Response_Log_Output", _medicName, _log1];
