...
if (isNil { player getVariable "CreditsChecked"; } ) then {
	...
	private _ProfileCredits = profileNamespace getVariable ["DPSOCS", 0];	// Get credits from Client Profile, if not set join with 500
	...
	[
		{
			...
			profileNameSpace setvariable ["DPSOCS", _WLFunds];			// Update Player Profile with credit amount
