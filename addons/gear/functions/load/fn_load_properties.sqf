 ...
 *	\z\dpso\addons\gear\functions\load\fn_load_properties.sqf
 *	by Ojemineh
 *
 *	load properties
 *
 ...
 *
 *	Example:
 *	[player, "curator"] call DPSO_gear_fnc_load_properties;
 *
 */
...
if (Not local _unit) exitWith {
	[_unit, _role, _type, _team] remoteExecCall ["DPSO_gear_fnc_load_properties", _unit];
};
...
if (isNil "_data") exitWith {[4, "Can't read unit-properties from file (%1)", [_file], "gear"] call DPSO_fnc_log;};
...
if (_data_insignia != "") then {
	[_unit, _data_insignia] call DPSO_fnc_setUnitInsignia;
};
...
if (count _data_safety > 0) then {
	...
	if (_safety_primary > 0) then {
		[_unit, (primaryWeapon _unit), (primaryWeapon _unit), false] call DPSO_fnc_weaponLock;
	};

	private _safety_secondary = (_data_safety select 1);
	if (_safety_secondary > 0) then {
		[_unit, (secondaryWeapon _unit), (secondaryWeapon _unit), false] call DPSO_fnc_weaponLock;
	};

	private _safety_handgun = (_data_safety select 2);
	if (_safety_handgun > 0) then {
		[_unit, (handgunWeapon _unit), (handgunWeapon _unit), false] call DPSO_fnc_weaponLock;
	};
