 ...
 * \z\dpso\addons\interactions\functions\tapshoulder\fn_tapshoulder_doTap.sqf
 * by Ojemineh
 *
 ...
 *
 * Example:
 * [player, unit1, 0] call DPSO_interactions_fnc_tapshoulder_doTap;
 *
 */
...
[_player, _target, _shoulder] spawn {
    ...
    if (missionNamespace getVariable ["dpso_interactions_global_sounds", false]) then {
        ...
        ["dpso_Action_Shoulder_Tap_1", _target, AGLToASL (_target modelToWorld (_target selectionPosition _selectionName)), 20, 1, _pitch, false] call DPSO_common_fnc_playSound3d;
    } else {
        playSound "dpso_Action_Shoulder_Tap_1";
    };

    if (isPlayer _target) then {
        [_player, _target, _shoulder] remoteExecCall ["dpso_interactions_fnc_tapshoulder_tap", _target];
    };
