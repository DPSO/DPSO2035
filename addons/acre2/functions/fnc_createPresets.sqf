...
private _calcChanFreq = {
    ...
    // e.g. [0,1,1] call dpso_acre2_calcChanFreq
...
dpso_acre2_radioBasenameToSettingsIdx = {
    params ["_radio"];

    private _fndIdx = GVAR(radioCoreSettings) findIf {_radio in (_x select 0)};

    if (_fndIdx isEqualTo -1) then { 
        systemChat format["Warning - DPSO ACRE2 Assign - GVAR(radioCoreSettings) missing radio: %1",_radio];
    };
    _fndIdx
};
...
//TODO Preserve shared channel indexes.
{
    ...
    //for "_i" from 1 to (count _radioPresetSetting)-1 do {
    {
        ...
        private _coreRadioIdx = [_radio] call dpso_acre2_radioBasenameToSettingsIdx;
    ...
    private _presetName = format["DPSO_preset%1",_radioPresetSettingIndex];
    ...
    private _channelName = "";
    {
        ...
        // Create Presets....
        {
            ...
            if (!_return) then {
                private _radioPresetData = [] call acre_main_fnc_fastHashCreate;//HASH_CREATE;
...
dpso_acre2_networksCreated = true;