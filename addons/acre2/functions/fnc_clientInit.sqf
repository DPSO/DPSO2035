...
[{
    ...
    if (getMissionConfigValue ['DPSO_AcreBabelEnabled',false]) then {
        ...
        private _groupCond = _unitGroup getVariable ["DPSO_BabelLanguages", []];
        if (_groupCond isEqualType "") then { _groupCond = call compile _groupCond; };
        private _unitCond = _unit getVariable ["DPSO_BabelLanguages", []];
    ...
    if (!getMissionConfigValue ['DPSO_AcreNetworkEnabled',false]) exitWith {
        if (getMissionConfigValue ['DPSO_AcreBabelEnabled',false]) then {
    ...
    private _unitVarChannelList = _unit getVariable ["DPSO_Channellist", []];
    if (_unitVarChannelList isEqualType "") then { _unitVarChannelList = call compile _unitVarChannelList; };
    private _groupVarChannelList = _unitGroup getVariable ["DPSO_Channellist", []];
    if (_groupVarchannelList isEqualType "") then { _groupVarchannelList = call compile _groupVarchannelList; };
    private _groupVarLeaderChannelList = _unitGroup getVariable ["DPSO_ChannellistLeader", []];
    ...
    private _isGroupLeader = (leader _unitGroup == _unit);
    {
        ...
        private _groupCond = _unitGroup getVariable ["DPSO_Network",-1];
        if (_groupCond == _radioPresetIdx) then { _passes = true; };
        private _unitCond = _unit getVariable ["DPSO_Network",-1];
        ...
        if (_passes) exitWith {
            ...
            _presetName = format["DPSO_preset%1",_ourPresetIndex];
            {
...

            {
                ...
                (_channelsProcessed select ([_radio] call dpso_acre2_radioBasenameToSettingsIdx)) pushBack (_x + [_unitOnThisChannel]);
            } forEach ((_radioPresetSetting select 1)); // loop through channels for the preset
        };
    } forEach GVAR(networksWithRadioChannels);
    if (_ourPresetIndex == -1) then { systemChat "[Warning] DPSO-ACRE no preset assigned for myself"; };
    ...
    private _side = _unit getVariable ["DPSO_assignGear_side",-1];
    private _faction = _unit getVariable ["DPSO_assignGear_faction","civ_f"];
    private _role =  _unit getVariable ["DPSO_assignGear_role","baseMan"];
   ...
   //  test = [_side, player getVariable ["DPSO_assignGear_faction","civ_f"], player getVariable ["DPSO_assignGear_role","baseMan"], "radios"]  call dpso_common_fnc_getCfgEntry;
    //test = [_side, player getVariable ["DPSO_assignGear_faction","civ_f"], player getVariable ["DPSO_assignGear_role","baseMan"], "radios"]  call dpso_common_fnc_getCfgEntry;
            ...
            // Allocate new radios.
            {
                if (_unit canAdd _x) then {
                    ...
                    if (!_added) then {
                        [_x] spawn {
                            waitUntil{time>3};
                            systemChat format["[DPSO ACRE2] Warning: No room to add radio '%1', report this to the mission maker. You now have an addaction to add the radio.",_this select 0];
                        };
                        ...
                        private _actionID = _unit addAction [format ["<t color='#3375D6'>[Radios] Give myself a %1 radio</t>",_radioName],
                        {
                            ...
                            if (_unit canAdd _radioToGive) then {
                                ...
                                systemChat format["[DPSO ACRE2] Warning: No room to add radio '%1', remove more stuff and try again",_radioToGive];
                            };
                        }
                ...
                if (!isNil QGVAR(radioAddActions)) then {
                    {
                        private _actionid_action = _unit addAction [format["<t color='#c3d633'>[Radios] Give myself a %1 radio (check your inventory for space)</t>",getText (configfile >> "CfgWeapons" >> _x >> "displayName")],
                                                            format["if ((_this select 0) canAdd '%1') then { (_this select 0) addItem '%1'; (_this select 0) removeAction (_this select 2); } else { systemChat '[DPSO ACRE2] No space for radios'; };",_x],0,0,false,true,"","(_target == _this)"];
