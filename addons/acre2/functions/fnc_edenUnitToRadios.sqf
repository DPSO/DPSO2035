...
/*
 * Name: DPSO_acre2_fnc_edenUnitToRadios
 * Author: Snippers
 *
 * Arguments:
 * 0: The Unit <OBJECT>
...
if (GVAR(giveMissingRadios)) then {
    ...
    private _networksWithRadioChannels = getMissionConfigValue ['DPSO_AcreSettings', []];
    ...
    if (_ourPresetIndex != -1) then {       
        private _unitVarChannelList = (_unit get3DENAttribute "DPSO_Channellist") param [0,[]];
        if (_unitVarChannelList isEqualType "") then { _unitVarChannelList = call compile _unitVarChannelList; };
        private _groupVarChannelList = (_unitGroup get3DENAttribute "DPSO_Channellist") param [0,[]];
        if (_groupVarchannelList isEqualType "") then { _groupVarchannelList = call compile _groupVarchannelList; };
        private _groupVarLeaderChannelList = (_unitGroup get3DENAttribute "DPSO_ChannellistLeader") param [0,[]];
