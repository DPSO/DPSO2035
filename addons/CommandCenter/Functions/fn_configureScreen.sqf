...
DPSO_CC_currentScreenObject = _object;
DPSO_CC_currentScreenSelection = _selection;
...
(_display displayCtrl 2400) ctrlSetEventHandler ["ButtonClick", "[] call DPSO_fnc_viewHelmetCam"];
(_display displayCtrl 2401) ctrlSetEventHandler ["ButtonClick", "[0] call DPSO_fnc_viewDroneCam"];
(_display displayCtrl 2402) ctrlSetEventHandler ["ButtonClick", "[] call DPSO_fnc_viewSatImage"];
(_display displayCtrl 2403) ctrlSetEventHandler ["ButtonClick", "[1] call DPSO_fnc_viewDroneCam"];
...
// set title
private _screenMode = _object getVariable [format ["DPSO_CC_screen_%1_mode", _selection], ""];
private _screenTarget = _object getVariable [format ["DPSO_CC_screen_%1_target", _selection], ""];
private "_modeName";
...
{
	private _name = "";
	private _section = _x getVariable "DPSO_Section";
...

{
	...
	_index = _hCamList lbAdd _name;
	_hCamList lbSetData [_index, str _x];
} forEach DPSO_CC_vehicleCamList;
