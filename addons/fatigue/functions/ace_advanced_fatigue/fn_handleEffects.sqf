 ...
 *	\z\dpso\addons\fatigue\functions\ace_advanced_fatigue\fn_handleEffects.sqf
 *	by Ojemineh
 *	
...
private _globalSounds = (missionNamespace getVariable ["dpso_fatigue_global_sounds", false]);
...
if (isNil "dpso_advanced_fatigue_breathVoice") then { 
	dpso_advanced_fatigue_breathVoice = selectRandom [0,1,2,3,4,5,6,7];
	if (_globalSounds) then {[5, "Use Global Sounds", [], "fatigue"] call DPSO_fnc_diagLog;};
	[5, "Selected Voice: %1", [dpso_advanced_fatigue_breathVoice], "fatigue"] call DPSO_fnc_diagLog;
};
...
if (_fatigue > 0.4 && {ace_advanced_fatigue_lastBreath > (_fatigue * -10 + 9)} && {!underwater _unit}) then {
    switch (true) do {
        case (_fatigue < 0.6): {
			_breathSound = "DPSO_Advanced_Fatigue_BreathLow";
			if (_pain > _painSuppress) then {_breathSound = ["DPSO_Advanced_Fatigue_BreathLow", "DPSO_Advanced_Fatigue_BreathInjuredLow"] select ((random 1) > 0.66);};
			if (_globalSounds) then {
				[_breathSound + format ["_%1_%2", dpso_advanced_fatigue_breathVoice, selectRandom [0,1,2,3,4]], _unit, AGLToASL (_unit modelToWorld (_unit selectionPosition "head")), 60, 1.4, -1, false] call DPSO_fnc_playSound3d;
			} else {
				playSound (_breathSound + format ["_%1_%2", dpso_advanced_fatigue_breathVoice, selectRandom [0,1,2,3,4]]);
			};
        };
        case (_fatigue < 0.85): {
			_breathSound = "DPSO_Advanced_Fatigue_BreathMid";
			if (_pain > _painSuppress) then {_breathSound = ["DPSO_Advanced_Fatigue_BreathMid", "DPSO_Advanced_Fatigue_BreathInjuredMid"] select ((random 1) > 0.66);};
			if (_globalSounds) then {
				[_breathSound + format ["_%1_%2", dpso_advanced_fatigue_breathVoice, selectRandom [0,1,2,3,4]], _unit, AGLToASL (_unit modelToWorld (_unit selectionPosition "head")), 60, 1.4, -1, false] call DPSO_fnc_playSound3d;
			} else {
				playSound (_breathSound + format ["_%1_%2", dpso_advanced_fatigue_breathVoice, selectRandom [0,1,2,3,4]]);
        ...
        default {
			_breathSound = "DPSO_Advanced_Fatigue_BreathMax";
			if (_pain > _painSuppress) then {_breathSound = ["DPSO_Advanced_Fatigue_BreathMax", "DPSO_Advanced_Fatigue_BreathInjuredMax"] select ((random 1) > 0.66);};
			if (_globalSounds) then {
				[_breathSound + format ["_%1_%2", dpso_advanced_fatigue_breathVoice, selectRandom [0,1,2,3,4]], _unit, AGLToASL (_unit modelToWorld (_unit selectionPosition "head")), 60, 1.4, -1, false] call DPSO_fnc_playSound3d;
			} else {
				playSound (_breathSound + format ["_%1_%2", dpso_advanced_fatigue_breathVoice, selectRandom [0,1,2,3,4]]);
