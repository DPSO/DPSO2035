...
//================== MASTER SWITCHES
private _QS_ST_map_enableUnitIcons = uiNamespace getVariable ["DPSO_QS_ST_map_enableUnitIcons", true];							// BOOL. TRUE to enable MAP unit/vehicle Icons. Default TRUE.
private _QS_ST_gps_enableUnitIcons = uiNamespace getVariable ["DPSO__QS_ST_gps_enableUnitIcons", true];							// BOOL. TRUE to enable MAP unit/vehicle Icons. Default TRUE.
private _QS_ST_enableGroupIcons = uiNamespace getVariable ["DPSO_QS_ST_enableGroupIcons", true];								// BOOL. TRUE to enable Map+GPS+HUD GROUP Icons. Default TRUE.

//================= ADMIN
private _QS_ST_admin = uiNamespace getVariable ["DPSO_QS_ST_admin", false];									// BOOL. TRUE to enable showing all units (even enemies) if logged in as admin on a server. Default FALSE;
private _QS_ST_showAll = uiNamespace getVariable ["DPSO_QS_ST_showAll", 0];									// NUMBER. Intended for Debug / Development use only! Caution: Will cause lag if 1 or 2! Settings -  0 = Disabled (Recommended). 1 = Reveal all Units + vehicles. 2 = Reveal all mission objects + vehicles + units. May override below configurations if set at 1 or 2.

//================= DIPLOMACY - set the Friendly factions for each faction.

private _QS_ST_friendlySides_Dynamic = uiNamespace getVariable ["DPSO_QS_ST_friendlySides_Dynamic", true];						// BOOL. Set TRUE to allow faction alliances to change dynamically (IE. AAF may not always be loyal to NATO) and be represented on the map. Default TRUE.
...
private _QS_ST_iconColor_EAST = uiNamespace getVariable ["DPSO_QS_ST_iconColor_EAST", [0.5,0,0,0.65]];
private _QS_ST_iconColor_WEST = uiNamespace getVariable ["DPSO_QS_ST_iconColor_WEST", [0,0.3,0.6,0.65]];
private _QS_ST_iconColor_RESISTANCE = uiNamespace getVariable ["DPSO_QS_ST_iconColor_RESISTANCE", [0,0.5,0,0.65]];
private _QS_ST_iconColor_CIVILIAN = uiNamespace getVariable ["DPSO_QS_ST_iconColor_CIVILIAN", [0.4,0,0.5,0.65]];
private _QS_ST_iconColor_UNKNOWN = uiNamespace getVariable ["DPSO_QS_ST_iconColor_UNKNOWN", [0.7,0.6,0,0.5]];

//================= MEDICAL

private _QS_ST_showMedicalWounded = uiNamespace getVariable ["DPSO_QS_ST_showMedicalWounded", true];						// BOOL. TRUE to show wounded on the map and GPS. FALSE to not show wounded on the map with this script. Default TRUE.
private _QS_ST_MedicalSystem = uiNamespace getVariable ["DPSO_QS_ST_MedicalSystem", 0];
...
private _QS_ST_MedicalIconColor = uiNamespace getVariable ["DPSO_QS_ST_MedicalIconColor", [1,0.41,0,1]];							// ARRAY (NUMBER). Color of medical icons in RGBA format. Default [1,0.41,0,1];
private _QS_ST_colorInjured = uiNamespace getVariable ["DPSO_QS_ST_colorInjured", [0.75,0.55,0,0.75]];						// ARRAY (NUMBER). RGBA color code. Color of units with > 10% damage, in map group interactive interface. Default [0.7,0.6,0,0.5];

//==================================================================================//
//=========================== CONFIGURE MAP (UNIT/VEHICLE) ICONS ===================//
//==================================================================================//

private _QS_ST_showFactionOnly = uiNamespace getVariable ["DPSO_QS_ST_showFactionOnly", true];								// BOOL. will override ST_showFriendlySides TRUE. If TRUE then will only show players faction. If FALSE then can show friendly factions. Default FALSE.
private _QS_ST_showAI = uiNamespace getVariable ["DPSO_QS_ST_showAI", false];												// BOOL. FALSE = players only, TRUE = players and AI. Default TRUE.
private _QS_ST_AINames = uiNamespace getVariable ["DPSO_QS_ST_AINames", false];												// BOOL. Set TRUE to show human names for AI with the map/vehicle icons. Set FALSE and will be named 'AI'. Default FALSE.
private _QS_ST_showCivilianIcons = uiNamespace getVariable ["DPSO_QS_ST_showCivilianIcons", false];							// BOOL. Set TRUE to allow showing of civilians, only works if Dynamic Diplomacy is enabled above. Default FALSE.
private _QS_ST_iconMapText = uiNamespace getVariable ["DPSO_QS_ST_iconMapText", true];										// BOOL. TRUE to show unit/vehicle icon text on the map. FALSE to only show the icon and NO text (name/class). Default TRUE.
private _QS_ST_showMOS = uiNamespace getVariable ["DPSO_QS_ST_showMOS", true];												// BOOL. TRUE = Show Military Occupational Specialty text(unit/vehicle class/role display name), FALSE = disable and only show icons and names. Default FALSE.
private _QS_ST_showMOS_range = uiNamespace getVariable ["DPSO_QS_ST_showMOS_range", 3500];									// NUMBER. Range in distance to show MOS on the map. Default 3500.
private _QS_ST_showGroupOnly = uiNamespace getVariable ["DPSO_QS_ST_showGroupOnly", true];									// BOOL. Set TRUE to Show ONLY the unit icons of THE PLAYERS GROUP MEMBERS on the MAP, FALSE to show ALL your factions units. May override other config. Default TRUE.
private _QS_ST_showOnlyVehicles = uiNamespace getVariable ["DPSO_QS_ST_showOnlyVehicles", false];							// BOOL. Set TRUE to show ONLY vehicles, no foot-soldier units will be shown. May override other config. Default TRUE.
private _QS_ST_iconMapClickShowDetail = uiNamespace getVariable ["DPSO_QS_ST_iconMapClickShowDetail", true];				// BOOL. Set TRUE to show unit/vehicle detail when player clicks on their map near the vehicle. Only works for shown vehicles. Default TRUE.
private _QS_ST_iconUpdatePulseDelay = uiNamespace getVariable ["DPSO_QS_ST_iconUpdatePulseDelay", 60];						// NUMBER. How often should location of unit on the MAP be updated? 0 = as fast as possible, else if > 0 then it = time in seconds. Default 0.
private _QS_ST_iconShadowMap = uiNamespace getVariable ["DPSO_QS_ST_iconShadowMap", 1];										// NUMBER. Icon Shadow on MAP. 0 = no shadow. 1 = shadow. 2 = outline. Must be 0, 1, or 2. Default 1.
private _QS_ST_iconTextSize_Map = uiNamespace getVariable ["DPSO_QS_ST_iconTextSize_Map", 0.05];							// NUMBER. Icon Text Size on MAP display. Default is 0.05.
private _QS_ST_iconTextOffset = uiNamespace getVariable ["DPSO_QS_ST_iconTextOffset", 'right'];								// STRING. Icon Text Offset. Can be 'left' or 'center' or 'right'. Default is 'right'
private _QS_ST_iconSize_Man = uiNamespace getVariable ["DPSO_QS_ST_iconSize_Man", 22];										// NUMBER. Icon Size by Vehicle Type. Man/Units. Default = 22
private _QS_ST_iconSize_LandVehicle = uiNamespace getVariable ["DPSO_QS_ST_iconSize_LandVehicle", 26];						// NUMBER. Icon Size by Vehicle Type. Ground-based vehicles. Default = 26	
private _QS_ST_iconSize_Ship = uiNamespace getVariable ["DPSO_QS_ST_iconSize_Ship", 24];									// NUMBER. Icon Size by Vehicle Type. Water-based vehicles. Default = 24
private _QS_ST_iconSize_Air = uiNamespace getVariable ["DPSO_QS_ST_iconSize_Air", 24];										// NUMBER. Icon Size by Vehicle Type. Air vehicles. Default = 24
private _QS_ST_iconSize_StaticWeapon = uiNamespace getVariable ["DPSO_QS_ST_iconSize_StaticWeapon", 22];

private _QS_ST_iconTextFonts = uiNamespace getVariable ["DPSO_QS_ST_iconTextFonts", 5];
...
private _QS_ST_otherDisplays = uiNamespace getVariable ["DPSO_QS_ST_otherDisplays", true];									// BOOL. TRUE to add Unit/Vehicle Icon support for UAV Terminal and Artillery Computer. Runs a separate script to handle these displays. Only works if  _QS_ST_map_enableUnitIcons = TRUE;
private _QS_ST_MAPrequireGPSItem = uiNamespace getVariable ["DPSO_QS_ST_MAPrequireGPSItem", false];							// BOOL. TRUE to require player have GPS in his assigned items. Default FALSE.

//==================================================================================//
//=========================== CONFIGURE GPS (UNIT/VEHICLE) ICONS ===================//
//==================================================================================//

private _QS_ST_GPSDist = uiNamespace getVariable ["DPSO_QS_ST_GPSDist", 300];												// NUMBER. Distance from player that units shown on GPS. Higher number = lower script performance. Not significant but every 1/10th of a frame counts! Default 300
private _QS_ST_GPSshowNames = uiNamespace getVariable ["DPSO_QS_ST_GPSshowNames", false];									// BOOL. TRUE to show unit names on the GPS display. Default FALSE.
private _QS_ST_GPSshowGroupOnly = uiNamespace getVariable ["DPSO_QS_ST_GPSshowGroupOnly", false];							// BOOL. TRUE to show only group members on the GPS display. Default TRUE.
private _QS_ST_iconTextSize_GPS = uiNamespace getVariable ["DPSO_QS_ST_iconTextSize_GPS", 0.05];							// NUMBER. Icon Text Size on GPS display. Default is 0.05.
private _QS_ST_iconShadowGPS = uiNamespace getVariable ["DPSO_QS_ST_iconShadowGPS", 1];										// NUMBER. Icon Shadow on GPS. 0 = no shadow. 1 = shadow. 2 = outline. Must be 0, 1, or 2. Default 1.
private _QS_ST_GPSrequireGPSItem = uiNamespace getVariable ["DPSO_QS_ST_GPSrequireGPSItem", false];							// BOOL. TRUE to require player have GPS in his assigned items. Default FALSE.

//==================================================================================//
//============================= CONFIGURE GROUP ICONS ==============================//
//==================================================================================//

private _QS_ST_showGroupMapIcons = uiNamespace getVariable ["DPSO_QS_ST_showGroupMapIcons", true];									// BOOL. Group icons displayed on map. Default TRUE.
private _QS_ST_showGroupHudIcons = uiNamespace getVariable ["DPSO_QS_ST_showGroupHudIcons", false];									// BOOL. Group icons displayed on player 3D HUD. Default FALSE.
private _QS_ST_showAIGroups = uiNamespace getVariable ["DPSO_QS_ST_showAIGroups", true];											// BOOL. Show Groups with AI leaders. Default TRUE.
private _QS_ST_showAINames = uiNamespace getVariable ["DPSO_QS_ST_showAINames", false];												// BOOL. Show AI Names. If FALSE, when names are listed with Group features, will only display as '[AI]'. Default FALSE.
private _QS_ST_groupInteractiveIcons = uiNamespace getVariable ["DPSO_QS_ST_groupInteractiveIcons", true];							// BOOL. Group icons are interactable (mouse hover and mouse click for group details). Default TRUE.
private _QS_ST_groupInteractiveIcons_showClass = uiNamespace getVariable ["DPSO_QS_ST_groupInteractiveIcons_showClass", true];		// BOOL. TRUE to show units vehicle class when revealing group details with interactive map group click. Default TRUE.
...
private _QS_ST_showGroupMapText = uiNamespace getVariable ["DPSO_QS_ST_showGroupMapText", true];									// BOOL. TRUE to show Group Name on the map. If FALSE, name can still be seen by clicking on the group icon, if QS_ST_groupInteractiveIcons = TRUE. Default FALSE.
private _QS_ST_groupIconScale = uiNamespace getVariable ["DPSO_QS_ST_groupIconScale", 0.75];										// NUMBER. Group Icon Scale. Default = 0.75
private _QS_ST_groupIconOffset = uiNamespace getVariable ["DPSO_QS_ST_groupIconOffset", [0.65,0.65]];								// ARRAY (NUMBERS). [X,Y], offset position of icon from group leaders position. Can be positive or negative numbers. Default = [0.65,0.65];
private _QS_ST_groupTextFactionOnly = uiNamespace getVariable ["DPSO_QS_ST_groupTextFactionOnly", true];							// BOOL. TRUE to show group icon text from ONLY the PLAYERS faction. FALSE will show text for all friendly/revealed factions. Default TRUE.
private _QS_ST_showCivilianGroups = uiNamespace getVariable ["DPSO_QS_ST_showCivilianGroups", false];								// BOOL. TRUE to show Civilian groups. Must be whitelisted above in friendlySides. Default FALSE.
private _QS_ST_showOwnGroup = uiNamespace getVariable ["DPSO_QS_ST_showOwnGroup", false];											// BOOL. TRUE to show the Players own group icon. Default FALSE.
private _QS_ST_GRPrequireGPSItem = uiNamespace getVariable ["DPSO_QS_ST_GRPrequireGPSItem", false];									// BOOL. TRUE to require player have GPS in his assigned items. Default FALSE.

//==================================================================================//
//============================= CONFIGURE BONUS FEATURES ===========================//
//==================================================================================//

private _QS_ST_showEmptyVehicles = uiNamespace getVariable ["DPSO_QS_ST_showEmptyVehicles", false];									// BOOL. TRUE to mark certain unoccupied vehicles on the map. The vehicle must be assigned this variable:    <vehicle> setVariable ['QS_ST_drawEmptyVehicle',TRUE,TRUE];    Default FALSE.   Only works if  _QS_ST_map_enableUnitIcons = TRUE;
private _QS_ST_iconColor_empty = uiNamespace getVariable ["DPSO_QS_ST_iconColor_empty", [0.7,0.6,0,0.5]];							// ARRAY (NUMBERS). Color of unoccupied vehicles, in RGBA. Default = [0.7,0.6,0,0.5];
private _QS_ST_iconSize_empty = uiNamespace getVariable ["DPSO_QS_ST_iconSize_empty", 20];											// NUMBER. Icon size of unoccupied vehicles, if shown.
