...
/*
 * Name = DPSO_chat_fnc_cmndWhisper
 * Author = Freddo
 *
 * Syntaxes:
...
if (_arg isEqualTo "") exitWith {
    systemChat "DPSO Error: No argument passed. Command usage: #whisper <player> <message>";
};
...
if (_message isEqualTo "") exitWith {
    systemChat "DPSO Error: No message passed. Command usage: #whisper <player> <message>";
};
...
if (!isNull _unit) then {
    ...
    systemChat format ["DPSO: Whisper sent to %1", name _unit];
} else {
    systemChat FORMAT_1("DPSO Error: No unit found containing %1, or more than one found.", str _message);
};
