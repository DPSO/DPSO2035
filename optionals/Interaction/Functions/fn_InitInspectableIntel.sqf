...
// Add the action
[_object, _actionTitle, _diaryRecord select 0] remoteExec ["DPSO_fnc_SetIntelAction",0,true];
...
if (typeName _sharedWith == "STRING") then {
	_recipients = [];
	{
		if (side _x == west && {_x getVariable ["DPSO_section", "ds990"] == _sharedWith}) then {
