...
if (_player getVariable ["cbrn_using_threat_meter", false]) then {
    if (isNull (uiNamespace getVariable ["cbrn_threatBaseCtrl", objNull])) then {
        ...
        _ctrl ctrlSetText QPATHTOF(Scripts\cbrn\images\threatmetercolours.paa);
        ...
        _ctrl ctrlSetText  QPATHTOF(Scripts\cbrn\images\needle.paa);
        _ctrl ctrlSetTextColor [1,1,1,1];
        _ctrl ctrlCommit 0;
        ...
        _ctrl ctrlSetPosition _pos;
        _ctrl ctrlSetBackgroundColor [1,1,1,1];
        _ctrl ctrlSetText  QPATHTOF(Scripts\cbrn\images\threatmeteroverlay.paa);
        _ctrl ctrlSetTextColor [1,1,1,1];
        _ctrl ctrlCommit 0;
