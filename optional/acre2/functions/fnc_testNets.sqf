...
/* ----------------------------------------------------------------------------
Function: DPSO_acre2_fnc_testNets

Description:
    Checks if players have no radio nets assigned to them.
    Used with DPSO Autotest.
...
if (getmissionconfigvalue ["DPSO_AcreNetworkEnabled",false]) then {
    LOG("Running Autotest on Radio Networks");
    private _networksArray = getMissionConfigValue ['DPSO_AcreSettings', []];
...

    {
        ...
        if (_index == -1) then {
            ...
            private _groupCond = _grp getVariable ["DPSO_Network", -1];
