...
// with uiNamespace do {     set3DENMissionAttributes [["DPSOMissionAcreAttributes","DPSO_AcreSettings",str RadioChannelArray]];    };
// with uiNamespace do {     set3DENMissionAttributes [["DPSOMissionAcreAttributes","DPSO_AcreSettings","[]"]];    };
...
fn_removeUnitFromChannel = {
    ...
    _list = (_unit get3DENAttribute "DPSO_Channellist") params ["_value"];
    if (_value isEqualType []) then {
        //Is default do nothing
        //_unit set3DENAttribute ["DPSO_Channellist",str []];
    } else {
        _value = call compile _value;
        _value = _value - [_channel];
        _unit set3DENAttribute ["DPSO_Channellist",str _value];
    };
};
...
fn_removeGroupFromChannel = {
    ...
    _list = (_group get3DENAttribute "DPSO_ChannellistLeader") params ["_value"];
    if (_value isEqualType []) then {
        //Is Default do nothing
        ...
        _group set3DENAttribute ["DPSO_ChannellistLeader",str _value];
    };

    
    _list = (_group get3DENAttribute "DPSO_Channellist") params ["_value"];
    if (_value isEqualType []) then {
        //is default do nothing
        ...
        _value = call compile _value;
        _value = _value - [_curSel];
        _group set3DENAttribute ["DPSO_Channellist",str _value];
    };
    // do units
    {
...
fn_networkTreeHandleKey = {
    ...
    if (_thing isEqualType west) then {
        private _sideNum = _thing call DPSO_common_fnc_sideToNum;
        {
            private _channel = _x;
        ...
        {
            set3DENAttributes [[units _x, "DPSO_Network", -1], [[_x],"DPSO_Network", -1]];
            set3DENAttributes [[units _x, "DPSO_Channellist", "[]"], [[_x],"DPSO_Channellist", "[]"], [[_x],"DPSO_ChannellistLeader", "[]"]];
    ...
    if (_thing isEqualType "") then {
        ...
        private _side = _sideNum call DPSO_common_fnc_numToSide;
    ...
    if (_thing isEqualType grpNull) then {
        ...
        set3DENAttributes [[units _thing, "DPSO_Network", -1],
            [[_thing],"DPSO_Network",(_networkNumber-1)]];
        set3DENAttributes [[units _thing, "DPSO_Channellist", "[]"], [[_thing],"DPSO_Channellist", "[]"], [[_thing],"DPSO_Channellist", "[]"], [[_thing],"DPSO_ChannellistLeader", "[]"]];
    ...
    if (_thing isEqualType objNull) then {
        ...
        set3DENAttributes [[[_thing],"DPSO_Network", (_networkNumber-1)],
            [[group _thing],"DPSO_Network",-1]];
        set3DENAttributes [[[_thing], "DPSO_Channellist", "[]"], [[group _thing],"DPSO_Channellist", "[]"], [[group _thing],"DPSO_ChannellistLeader", "[]"]];
...
switch _mode do {
    case "onLoad": {
        ...
        RadioChannelArray = ("DPSO_MissionAcre2Attributes" get3DENMissionAttribute "DPSO_AcreSettings");
        ...
        if (isNil "RadioChannelArray") then {
            ...
            RadioChannelArray = [];
            //RadioChannelArray = call compile ("DPSOMissionAcreAttributes" get3DENMissionAttribute "DPSO_AcreSettings");
        };
        ...
        //Deserialize: Convert nums to side from
        {
            ...
            } forEach (_conditions);
            {
                ...
                {
                    if (_x isEqualType 0) then {
                        _condition set [_forEachIndex,(_x call DPSO_common_fnc_numToSide)];
                    };
                } forEach (_condition);
            } forEach (_channels);
        } forEach RadioChannelArray;
    ...
    case "attributeSave": {
        ...
        private _array = + (uiNamespace getVariable "RadioChannelArray");
        {
            ...
            } forEach (_conditions);
            {
                ...
                {
                    if (_x isEqualType east) then {
                        _condition set [_forEachIndex, (_x call DPSO_common_fnc_sideToNum)];
                    };
                } forEach (_condition);
            } forEach (_channels);
        } forEach _array;
    ...
    case "refreshChannelTree": {
        ...
        fn_channelTreeProcessUnit = {
            ...
            if (!_giveRadio) then {
                private _unitChanList = (_unit get3DENAttribute "DPSO_Channellist") select 0;
                if (_unitChanList isEqualType "") then {
                    _unitChanList = call compile _unitChanList;
                };
                //private _unitChanList = (call compile ((_unit get3DENAttribute "DPSO_Channellist") select 0));
            ...
            if (_giveRadio) then {
                _ctrlTree tvSetPictureRight [_location, "z\dpso\addons\acre2\ui\RadioIcon_Radio_Small.paa"];
                _returnCode = 0;
            } else {
                _ctrlTree tvSetPictureRight [_location, "z\dpso\addons\acre2\ui\RadioIcon_None_Small.paa"];
            };
        ...
        fn_channelTreeProcessGroup = {
            ...
            if (!_giveRadio) then {
                private _grpChanList = (_group get3DENAttribute "DPSO_Channellist") select 0;
                ...
                if (RadioCurrentNetworkChannel in _grpChanList) then {
                    ...
                    _grpChanList = (_group get3DENAttribute "DPSO_ChannellistLeader") select 0;
                    if (_grpChanList isEqualType "") then {
                        _grpChanList = call compile _grpChanList;
                    };
                    if (RadioCurrentNetworkChannel in _grpChanList) then {
            ...
            if (_giveRadio) then {
                ...
                if (_giveLeader) then {
                    _ctrlTree tvSetPictureRight [_location, "z\dpso\addons\acre2\ui\RadioIcon_Leader_Small.paa"];
                    _returnCode = 2;
                } else {
                    if (_gaveSomeoneARadio) then {
                        _ctrlTree tvSetPictureRight [_location, "z\dpso\addons\acre2\ui\RadioIcon_Partial_Small.paa"];
        ...
        
        {
            ...
            if (_sides find (side _group) == -1) then {
                if (_sides find (toLower faction (leader _group)) == -1) then {
                    if (((_group get3DENAttribute "DPSO_Network") select 0) == RadioCurrentNetwork) then {
                        [_ctrlTree, [], false, _x] call fn_channelTreeProcessGroup;
                    } else {
                        {
                            if (((_x get3DENAttribute "DPSO_Network") select 0) == RadioCurrentNetwork) then {
    ...
    case "refreshNetworkTree": {
        ...
        _factions = [];
        {
            ...
            //Find Side
            if (_sideIdx == -1) then {
                private _sideText = _side call DPSO_common_fnc_sideToString; 
                private _sideIcon = _side call DPSO_common_fnc_sideToTexture; 
                ...
                if (_networkNumber != -1) then {
                    _ctrlTree tvSetPictureRight [[_sideIdx], (_networkNumber+1) call DPSO_common_fnc_numToTexture];
                } else {
            ...
            if (_factionIdx == -1) then {
                ...
                if (_networkNumber != -1) then {
                    _ctrlTree tvSetPictureRight [[_sideIdx, _factionIdx], (_networkNumber+1) call DPSO_common_fnc_numToTexture];
                } else {
            ...
            if (_networkNumber != -1) then {
                _ctrlTree tvSetPictureRight [[_sideIdx, _factionIdx, _grpIdx], (_networkNumber+1) call DPSO_common_fnc_numToTexture];
            } else {
                _networkNumber = (_group get3DENAttribute "DPSO_Network") select 0;
                if (_networkNumber != -1) then {
                    _ctrlTree tvSetPictureRight [[_sideIdx, _factionIdx, _grpIdx], (_networkNumber+1) call DPSO_common_fnc_numToTexture];
                };
            };
            ...
            private _preUnitNetworkNumber = _networkNumber;
            {
                ...
                if (_networkNumber != -1) then {
                    _ctrlTree tvSetPictureRight [[_sideIdx, _factionIdx, _grpIdx, _unitIdx], (_networkNumber+1) call DPSO_common_fnc_numToTexture];
                } else {
                    _networkNumber = (_x get3DENAttribute "DPSO_Network") select 0;
                    if (_networkNumber != -1) then {
                        _ctrlTree tvSetPictureRight [[_sideIdx, _factionIdx, _grpIdx, _unitIdx], (_networkNumber+1) call DPSO_common_fnc_numToTexture];
                    };
                };
    ...
    case "channelDelClick": {
        with uiNamespace do {
            ...
            private _units = allUnits select {([_x] call DPSO_acre2_fnc_unitToPreset) == 1};
            private _groups = []; {_groups pushBackUnique (group _x);} forEach _units;
            {
                private _entity = _x;
                private _list = (_entity get3DENAttribute "DPSO_Channellist") params [["_value",[]]];
                if (_value isEqualType "") then {
                    _value = call compile _value;
                };
                ...
                _entity set3DENAttribute ["DPSO_Channellist",str _value];
            } forEach (_units + _groups);
            // Cycle through groups to remove leader.
            {
                private _entity = _x;
                private _list = (_entity get3DENAttribute "DPSO_ChannellistLeader") params [["_value",[]]];
                if (_value isEqualType "") then {
                    _value = call compile _value;
                };
                _value = _value - [_curSel]; // Remove current channel.
                ...
                _entity set3DENAttribute ["DPSO_ChannellistLeader",str _value];
            } forEach _groups;
        ...
        with uiNamespace do {
            ...
            if (_entity isEqualType east or _entity isEqualType "") then {
                ...
                if (_entity isEqualType grpNull or _entity isEqualType objNull) then {
                    _list = (_entity get3DENAttribute "DPSO_Channellist") params ["_value"];
                    if (_value isEqualType []) then {
                        _entity set3DENAttribute ["DPSO_Channellist",str [_curSel]];
                    } else {
                        _value = call compile _value;
    ...
    case "channelTreeLeader": {
        with uiNamespace do {
            ...
            if (_entity isEqualType grpNull) then {
                _list = (_entity get3DENAttribute "DPSO_ChannellistLeader") params ["_value"];
                if (_value isEqualType []) then {
                    _entity set3DENAttribute ["DPSO_ChannellistLeader",str [_curSel]];
                } else {
                    _value = call compile _value;
                    _value pushBackUnique _curSel;
                    _entity set3DENAttribute ["DPSO_ChannellistLeader",str _value];
    ...
    case "save": {
        ...
        set3DENMissionAttributes [["DPSOMissionAcreAttributes", "DPSO_AcreSettings", _string]];
    };
};
