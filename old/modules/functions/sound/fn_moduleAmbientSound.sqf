...
switch _mode do {
	...
	case "init": {
		...
		if !((typeName _soundOffset) isEqualType "ARRAY") exitWith {
			...
			[2, "ModuleAmbientSound '%1' - %2", [_logic, format [localize "STR_DPSO_ModuleError_OffsetNotValid", _soundOffset]], "modules"] call DPSO_fnc_log;
		};
		...
		if (_source != "") then {
			...
			if (isNull (missionNamespace getVariable [_source, objNull])) exitWith {
				...
				[2, "ModuleAmbientSound '%1' - %2", [_logic, format [localize "STR_DPSO_ModuleError_VarIsUnknown", _source]], "modules"] call DPSO_fnc_log;
			};
			...
			if !((typeName _soundSource) isEqualType "OBJECT") exitWith {
				...
				[2, "ModuleAmbientSound '%1' - %2", [_logic, format [localize "STR_DPSO_ModuleError_IsNotObject", _soundSource]], "modules"] call DPSO_fnc_log;
			};
		...
		if (_soundExist isEqualTo 0) exitWith {
			...
			[2, "ModuleAmbientSound '%1' - %2", [_logic, format [localize "STR_DPSO_ModuleError_UnknownInCfgSounds", _sound]], "modules"] call DPSO_fnc_log;
		};
		...
		if (_distance <= 0) exitWith {
			...
			[2, "ModuleAmbientSound '%1' - %2", [_logic, format [localize "STR_DPSO_ModuleError_DistanceGreaterZero", _distance]], "modules"] call DPSO_fnc_log;
		};
		...
		if ((_maxDistance > 0) && (_maxDistance <= _distance)) exitWith {
			...
			[2, "ModuleAmbientSound '%1' - %2", [_logic, format [localize "STR_DPSO_ModuleError_MaxGreaterDistance", _distance, _maxDistance]], "modules"] call DPSO_fnc_log;
		};
		...
		if (_duration <= 0) exitWith {
			...
			[2, "ModuleAmbientSound '%1' - %2", [_logic, format [localize "STR_DPSO_ModuleError_DurationNotDefined", _sound]], "modules"] call DPSO_fnc_log;
		};
