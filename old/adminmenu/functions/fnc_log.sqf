...
/* ----------------------------------------------------------------------------
Function: DPSO_adminmenu_fnc_log
...
Parameters:
    ...
    _tag - Tag that message will be prefixed with [String, defaults to "[DPSO Log]"]
...
Examples:
    (begin example)
        ["Everything is on fire!",true,"[DPSO Firealarm]"] call DPSO_adminmenu_fnc_log;
    (end)
    (begin example)
        [format ["%1 did something",profileName]] call DPSO_adminmenu_fnc_log;
    (end)
