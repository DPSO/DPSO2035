...
if (isNil "DPSO_CC_nextScreenID") then {
	DPSO_CC_nextScreenID = 0;
};

_object setVariable [format ["DPSO_CC_screen_%1_ID", _selection], DPSO_CC_nextScreenID, true];

DPSO_CC_nextScreenID = DPSO_CC_nextScreenID + 1;

_object setVariable [format ["DPSO_CC_screen_%1_on", _selection], false, true];
_object setVariable [format ["DPSO_CC_screen_%1_mode", _selection], "", true];
_object setVariable [format ["DPSO_CC_screen_%1_target", _selection], "", true];

// set default screen texture
_object setObjectTextureGlobal [_selection, "z\dpso\addons\media\images\cc_screen_standby.paa"];

[_object, _selection, _name, [_allowCam, _allowDrone, _allowSat, _allowMap]] remoteExec ["DPSO_fnc_addScreenActions", 0, true];