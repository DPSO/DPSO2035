...
/*
 * Name: DPSO_spectator_fnc_drawGroupMarker
 * Author: Head
 *
 * Arguments:
...
params ['_map', '_grp','_color', '_pos'];
 private _dpsoGrpMarkerData = [_x] call EFUNC(orbat,getGroupMarkerData);
// if we have a framework orbat marker, use it.
if(count _dpsoGrpMarkerData >= 3) then {
    _dpsoGrpMarkerData params ["_grpTexture","_gname","_grpTextureSize"];
