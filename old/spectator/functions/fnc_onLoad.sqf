...
with uiNamespace do {
    ...
    GVAR(unitlabel) = _display displayCtrl IDC_SPECTATOR_DPSO_SPECTATOR_UNITLABEL;
    GVAR(unitlist) = _display displayCtrl IDC_SPECTATOR_DPSO_SPECTATOR_UNITLIST;

    GVAR(vision) = _display displayCtrl IDC_SPECTATOR_DPSO_SPECTATOR_VISION;

    GVAR(filter) = _display displayCtrl IDC_SPECTATOR_DPSO_SPECTATOR_FILTER;
    GVAR(side) = _display displayCtrl IDC_SPECTATOR_DPSO_SPECTATOR_BUTTON;
    GVAR(tagsbutton) = _display displayCtrl IDC_SPECTATOR_DPSO_SPECTATOR_TAGS;
    GVAR(view) = _display displayCtrl IDC_SPECTATOR_DPSO_SPECTATOR_VIEW;
    GVAR(mute) = _display displayCtrl IDC_SPECTATOR_DPSO_SPECTATOR_MUTE;
    GVAR(radio) = _display displayCtrl IDC_SPECTATOR_DPSO_SPECTATOR_RADIO;
    GVAR(map) = _display displayCtrl IDC_SPECTATOR_DPSO_SPECTATOR_MAP;
    GVAR(compass) = [_display displayCtrl IDC_SPECTATOR_DPSO_SPECTATOR_COMPASSLEFT,_display displayCtrl IDC_SPECTATOR_DPSO_SPECTATOR_COMPASS,_display displayCtrl IDC_SPECTATOR_DPSO_SPECTATOR_COMPASSRight];
...
if(!GVAR(canSpectateAllSides)) then {
    GVAR(sides) = [dpso_spectator_entryside];
    GVAR(sides_button_mode) = [[dpso_spectator_entryside], []];
...
if (isClass(configFile >> "CfgPatches" >> "acre_main")) then {
    ...
    // Add all languages
    if (!isNil "dpso_acre2_languagesTable") then {
        private _languages = dpso_acre2_languagesTable apply {_x select 0};
