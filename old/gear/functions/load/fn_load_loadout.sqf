 ...
 *	\z\dpso\addons\gear\functions\load\fn_load_loadout.sqf
 *	by Ojemineh
 *
 *	load loadout
 *
 ...
 *
 *	Example:
 *	[player, "curator"] call DPSO_gear_fnc_load_loadout;
 *
 */
...
if (Not local _unit) exitWith {
	[_unit, _role, _type, _team] remoteExecCall ["DPSO_gear_fnc_load_loadout", _unit];
};
...
if (isNil "_data") exitWith {[4, "Can't read unit-loadout from file (%1)", [_file], "gear"] call DPSO_fnc_log;};
[5, "'%1' loading gear '%2' (File: %3) - Data: %4", [_unit, _role, _file, _data], "gear"] call DPSO_fnc_diagLog;
...
if (GEAR_KEEP_GOGGLES) then {
	private _unit_goggles = _unit getVariable ["DPSO_gear_goggles", ""];
	if (_unit_goggles != "") then {
		if ((goggles _unit) != "") then {removeGoggles _unit;};
		_unit addGoggles _unit_goggles;
		_unit setVariable ["DPSO_gear_goggles", nil];
	};
};
